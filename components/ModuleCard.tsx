...
export const ModuleCard: React.FC<Props> = ({ module, onClick }) => {
  return (
    <button
      onClick={onClick}
      className="flex flex-col text-left bg-white dark:bg-slate-900 p-6 sm:p-8 rounded-[2rem] sm:rounded-[2.5rem] shadow-sm border border-slate-100 dark:border-slate-800/50 hover:shadow-2xl hover:shadow-emerald-500/5 transition-all duration-500 group hover:border-emerald-200 dark:hover:border-emerald-700 h-full relative overflow-hidden"
    >
      {/* Background soft glow in dark mode */}
      <div className="absolute top-0 right-0 w-32 h-32 bg-emerald-500/5 blur-[60px] pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-700"></div>

      <div className="flex justify-between items-start mb-6 sm:mb-8 w-full relative z-10">
        <div className="p-3.5 sm:p-4 bg-emerald-50 dark:bg-emerald-500/10 rounded-[1rem] sm:rounded-2xl group-hover:bg-emerald-600 dark:group-hover:bg-emerald-500 transition-all duration-500">
        ...
        <span className="text-[10px] font-bold text-slate-400 dark:text-slate-500 bg-slate-50 dark:bg-slate-800/50 px-3.5 py-1.5 rounded-full tracking-widest border border-slate-100 dark:border-slate-800/50 uppercase">
          {module.code}
        </span>
      </div>
