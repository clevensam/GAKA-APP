    ...
    <div className="animate-fade-in flex flex-col items-center">
      <div className="text-center pt-4 pb-16 lg:pt-32">
        ...
        <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto px-6 justify-center">
          ...
          <button onClick={() => onNavigate('about')} className="px-10 py-5 sm:px-16 sm:py-6 bg-white dark:bg-[#1E1E1E] text-slate-700 dark:text-white/80 border border-slate-200 dark:border-white/5 rounded-full font-bold text-sm sm:text-base hover:bg-slate-50 dark:hover:bg-[#282828] transition-all duration-300 shadow-sm active:scale-95">
            Learn More
          </button>
        </div>
      </div>
      {recentFiles.length > 0 && (
        <div className="w-full max-w-6xl mb-12 px-4 animate-fade-in">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-10">
            {recentFiles.map((file, idx) => <RecentFileCard key={file.id} file={file} delay={idx * 100} onNavigate={onNavigate} />)}
          </div>
        </div>
      )}
