...
export const ErrorPage: React.FC<ErrorPageProps> = ({ message, isRetrying }) => {
  const handleReload = () => {
    ...
    <div className="min-h-[75vh] flex flex-col items-center justify-center px-6 py-12 animate-fade-in text-center">
      <div className="relative mb-10 group">
        ...
        <div className="relative z-10 animate-float">
          <svg width="240" height="240" viewBox="0 0 240 240" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-56 h-56 sm:w-72 sm:h-72">
            ...
            <rect x="85" y="140" width="70" height="20" rx="4" className="stroke-red-500 dark:stroke-red-400" strokeWidth="4" />
            ...
            <circle cx="140" cy="150" r="3" className="fill-red-500 animate-ping" />
      ...
      <div className="max-w-2xl mx-auto space-y-5 mb-14">
        <h2 className="text-4xl sm:text-6xl font-black text-slate-900 dark:text-white leading-[1.1] tracking-tight">
          Server <span className="text-red-500 dark:text-red-400">Connection</span> Error
        </h2>
        
        <p className="text-slate-500 dark:text-white/40 text-base sm:text-xl font-medium leading-relaxed px-4">
          {message || "The academic registry is temporarily unreachable. Please check your connection."}
        </p>
      </div>

      <div className="flex flex-col items-center justify-center gap-5 w-full sm:w-auto px-6">
        <button 
          onClick={handleReload}
          disabled={isRetrying}
          className="w-full sm:w-auto group relative flex items-center justify-center px-16 py-6 bg-emerald-600 dark:bg-emerald-500 text-white rounded-[1.5rem] font-bold text-lg shadow-2xl shadow-emerald-200 dark:shadow-emerald-900/40 hover:bg-emerald-700 dark:hover:bg-emerald-600 hover:scale-[1.03] active:scale-95 transition-all duration-300 disabled:opacity-70"
        >
          {isRetrying ? (
      ...
      <div className="mt-16 flex flex-col items-center space-y-4 opacity-30">
        <p className="text-[11px] font-bold uppercase tracking-[0.25em] text-slate-400 dark:text-white/20">
          MUST CS Registry Status: Offline
        </p>
      </div>
    </div>
  );
