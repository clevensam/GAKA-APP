  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const baseModules = MODULES_DATA.map(m => ({ ...m, resources: [] }));
        ...
        rows.slice(1).forEach((row, i) => {
          ...
          const module = baseModules.find(
            m => m.code.replace(/\s/g, '').toLowerCase() ===
        ...
        setModules(baseModules.filter(m => m.resources.length));
        setError(null);
      } catch {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-grow container mx-auto px-6 py-10">
        {error && (
          <div className="mb-8 p-5 bg-amber-50 border border-amber-200 rounded-xl flex justify-between items-center">
